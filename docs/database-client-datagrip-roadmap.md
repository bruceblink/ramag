...
> 状态：M1 结果数据编辑器、M2-A SQL 查询上下文隔离、M2-B 最近关闭查询草稿恢复、M3-A SQL 手动事务提交/回滚、M3-B 事务失败状态与恢复提示、M4-A 原始执行计划结果视图、M4-B 只读 Schema Diagram 预览、M4-C 字段结构差异预览与 M4-D 同 Schema 表结构对比已落地，后续迭代中
> 更新日期：2026-08-25
...
- M2-A 已为 SQL 查询代次和 COUNT 代次增加上下文隔离；取消请求现在绑定数据/计划结果目标，不会清空另一面板或把旧的服务端取消提示写入新查询。连接、Schema 或 SQL 上下文失效时也会对旧 SQL 发起尽力取消。多标签并行执行和连接断开仍需继续补充真实环境集成验证。
