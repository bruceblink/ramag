...
3. **表树统计与对象导航**
   ...
   - 在表、列和查询编辑器之间建立稳定的定位动作：从表树打开数据/DDL，从 SQL 中选中对象后定位到导航树，并显示当前 Schema 和连接。
