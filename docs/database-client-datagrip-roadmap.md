...
> 状态：M1 结果数据编辑器、M2-A SQL 查询上下文隔离、M4-A 原始执行计划结果视图与 M4-B 只读 Schema Diagram 预览已落地，后续迭代中
> 更新日期：2026-08-23
...
- 当前已支持独立的原始 EXPLAIN 结果视图和只读 Schema Diagram 元数据预览；结构化计划树、图形化执行计划、数据/结构对比和迁移工作流仍在后续迭代中。
