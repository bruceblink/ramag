...
> 状态：M1 结果数据编辑器、M2-A SQL 查询上下文隔离、M2-B 最近关闭查询草稿恢复、M3-A SQL 手动事务提交/回滚、M3-B 事务失败状态与恢复提示、M4-A 原始/结构化执行计划结果视图、M4-B 只读 Schema Diagram 预览、M4-C 字段结构差异预览与 M4-D 同 Schema 表结构对比已落地，后续迭代中
> 更新日期：2026-08-26
...
- 当前已支持独立的 EXPLAIN 结果视图、MySQL/PostgreSQL 常见文本计划的结构化树、只读 Schema Diagram 元数据预览、表设计器中的字段级结构差异预览，以及同一 Schema 下两张 SQL 表的列/索引/外键结构对比；图形化执行计划、跨 Schema 对象对比和迁移工作流仍在后续迭代中。
