...
> 状态：M1 结果数据编辑器、M2-A SQL 查询上下文隔离、M4-A 原始执行计划结果视图、M4-B 只读 Schema Diagram 预览与 M4-C 字段结构差异预览已落地，后续迭代中
> 更新日期：2026-08-24
...
- 当前已支持独立的原始 EXPLAIN 结果视图、只读 Schema Diagram 元数据预览和表设计器中的字段级结构差异预览；结构化计划树、图形化执行计划、跨对象结构对比和迁移工作流仍在后续迭代中。
...
- 表结构和 DDL diff，先以表设计器字段级差异预览切片落地，再扩展到索引、外键和跨对象对比。
