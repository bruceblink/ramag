...
- SQL 和 MongoDB 查询页大小仍是代码常量，默认值为 `10000`，用户不能选择 `100/200/500/1000` 或自定义值。
- 结果表的纵向滚动体验仍需要真实窗口截图验证，确保列表高度、底部状态栏和垂直滚动条不会互相覆盖。
...
Ramag 应该采用 **“以 DataGrip 为交互参考、以核心工作流为产品边界、以里程碑逐步扩大能力”** 的策略。第一步不是追求图形化计划或 Schema Diagram，而是完成 M1 结果数据编辑器：默认 100 条分页、可选 page size、可靠的双轴滚动和真实窗口验证。M1 稳定后，再根据用户反馈决定是否投入查询会话、事务和高级分析能力。
