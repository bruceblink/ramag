...
> 状态：M1 结果数据编辑器、M2-A SQL 查询上下文隔离、M2-B 最近关闭查询草稿恢复、M3-A SQL 手动事务提交/回滚、M4-A 原始执行计划结果视图、M4-B 只读 Schema Diagram 预览、M4-C 字段结构差异预览与 M4-D 同 Schema 表结构对比已落地，后续迭代中
> 更新日期：2026-08-25
...
- 当前已支持独立的原始 EXPLAIN 结果视图、只读 Schema Diagram 元数据预览、表设计器中的字段级结构差异预览，以及同一 Schema 下两张 SQL 表的列/索引/外键结构对比；结构化计划树、图形化执行计划、跨 Schema 对象对比和迁移工作流仍在后续迭代中。
...
- 当前切片只覆盖列名、类型、NULL 属性、默认值和注释；索引、外键、表改名及跨表结构对比仍属于后续范围。
...
#### M4-D：同 Schema 表结构对比（已落地切片）

设计目标：在 Schema 浏览器的表右键菜单中选择源表和目标表，读取两张 SQL 表的真实元数据并生成只读差异视图，帮助用户在不执行 DDL 的前提下检查结构漂移。

实现边界：

- 仅支持 MySQL 和 PostgreSQL 的同一 Schema；MongoDB、Redis 和跨 Schema 对比保留为后续能力。
- 差异来源为驱动返回的 `Column`、`Index` 和 `ForeignKey`，不解析 UI 文本，也不执行任何迁移 SQL。
- 同名对象按大小写不敏感方式匹配；定义变化显示为删除旧定义和新增新定义，索引列顺序与外键引用列顺序保留。
- 对话框提供复制差异、刷新元数据以及水平/垂直滚动；单类元数据读取失败时保留其他结果并显示按源表/目标表区分的警告。
- 后台读取带请求代次和连接/Schema/表名校验，旧响应不能覆盖当前对比上下文。

验收标准：

1. 表右键菜单可以打开结构对比，输入目标表后显示列、索引和外键三组差异。
...
- 表结构和 DDL diff，已先落地表设计器字段级差异预览与同 Schema 表级列/索引/外键对比，再扩展到跨 Schema 对象对比和迁移工作流。
...
Ramag 应该采用 **“以 DataGrip 为交互参考、以核心工作流为产品边界、以里程碑逐步扩大能力”** 的策略。M1 结果数据编辑器已经完成默认 100 条分页、可选 page size 和可靠的双轴滚动；M3-A 已完成 SQL 手动事务的提交/回滚驱动链路和真实 MySQL/PostgreSQL 验证，后续继续补充原生窗口截图、事务失败恢复和更细的会话语义。
