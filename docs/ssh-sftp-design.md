...
│ 配置列表         │ 配置详情 / 新建 / 编辑           │
│                  │                                   │
│ production       │ 名称、颜色、主机、端口、用户     │
│ staging          │ 认证方式、密钥、初始目录         │
└──────────────────┴───────────────────────────────────┘
```
...
1. `alacritty_terminal` 的锁定版本和是否需要补充 PTY crate。
2. SFTP 最终使用“系统 SSH 子系统 + russh-sftp”还是完整 Rust SSH 栈。
3. 后续是否增加 SFTP 密码 / AskPass 认证。
4. Terminal 与 SFTP 是否值得通过 ControlMaster 复用底层连接。
5. 各类资源预算和默认并发数。

这些事项未验证前不得写成稳定 API 或对外功能承诺。
