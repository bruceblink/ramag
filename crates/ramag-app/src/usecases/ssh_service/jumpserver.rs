...
#[path = "jumpserver_connections.rs"]
mod jumpserver_connections;
...
impl SshService {
    pub async fn load_jumpserver_catalog(
        ...
        match &result {
            ...
            Err(load_error) => warn!(
                operation = "jumpserver_catalog_load",
                error = %load_error,
                "load jumpserver catalog failed"
            ),
        }
        result
    }
    ...
    pub async fn jumpserver_asset_detail(
        ...
        match &result {
            Ok(detail) => {
                info!(
                    operation = "jumpserver_asset_detail",
                    asset_id = %asset.id,
                    accounts = detail.accounts.len(),
                    active = detail.asset.active,
                    rdp_web = detail.rdp_web_enabled,
                    "jumpserver asset detail loaded"
                )
            }
            Err(detail_error) => {
                warn!(
                    ...
                    error = %detail_error,
                    asset_id = %asset.id,
                    "load jumpserver asset detail failed"
                )
            }
        }
        result
    ...
    pub async fn create_jumpserver_rdp_web_session(
        ...
        match &result {
            Ok(_) => info!(
                operation = "jumpserver_rdp_session_create",
                asset_id = %asset.id,
                "jumpserver rdp web session created"
            ),
            Err(session_error) => {
                warn!(
                    operation = "jumpserver_rdp_session_create",
                    error = %session_error,
                    asset_id = %asset.id,
                    "create jumpserver rdp web session failed"
                )
            }
        }
        result
    ...
    pub async fn test_jumpserver_asset(
        ...
        info!(
            operation = "jumpserver_asset_test",
            asset_id = %asset.id,
            profile_id = %profile.id,
            "jumpserver ssh asset test succeeded"
        );
        Ok(profile)
    }
    ...
    pub async fn save_jumpserver_asset(
        ...
        info!(
            operation = "jumpserver_asset_save",
            asset_id = %asset.id,
            profile_id = %profile.id,
            "jumpserver ssh asset saved"
        );
        Ok(profile)
    }
    ...
    pub async fn save_jumpserver_asset_for_connection(
        ...
        info!(
            ...
            asset_id = %asset.id,
            profile_id = %profile.id,
            "jumpserver asset saved for connection"
        );
        Ok(profile)
    }
