...
use super::{
    ...
    ObjectStorageService, SESSION_PREFERENCE_KEY, workspace_preference_key,
};
...
impl ObjectStorageService {
    ...
    pub async fn save_session_preference(
        ...
        validate_session_preference(preference)?;
        let plain = serde_json::to_vec(preference)
            .map_err(|error| DomainError::Storage(format!("序列化对象存储会话失败：{error}")))?;
        if plain.len() > MAX_OBJECT_STORAGE_SESSION_BYTES {
            return Err(DomainError::InvalidConfig(
                "对象存储会话明文超过安全上限".into(),
            ));
    ...
    pub async fn load_workspace(
        ...
        let encrypted = hex::decode(encoded)
            .map_err(|error| DomainError::Storage(format!("对象存储工作区密文无效：{error}")))?;
        let plain = self.storage.unseal(&encrypted).await?;
        ...
        let preference: ObjectStorageWorkspacePreference = serde_json::from_slice(&plain)
            .map_err(|error| DomainError::Storage(format!("对象存储工作区格式无效：{error}")))?;
    ...
    pub async fn save_workspace(
        ...
        let plain = serde_json::to_vec(preference)
            .map_err(|error| DomainError::Storage(format!("序列化对象存储工作区失败：{error}")))?;
        if plain.len() > MAX_OBJECT_STORAGE_WORKSPACE_BYTES {
            return Err(DomainError::InvalidConfig(
                "对象存储工作区明文超过安全上限".into(),
            ));
