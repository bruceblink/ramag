...
impl PoolCache {
    ...
    pub async fn get_or_create(
        ...
        if let Some(manager) = self.get_cached(&key, generation) {
            ...
            debug!(operation = "redis_pool_cache_lookup", connection_id = %config.id, db, "connection pool cache hit");
            return Ok(manager);
        }
        ...
        if let Some(manager) = self.get_cached(&key, generation) {
            self.enforce_pool_limit(&config.id, &key, generation);
            debug!(operation = "redis_pool_cache_lookup", connection_id = %config.id, db, after_lock = true, "connection pool cache hit");
            return Ok(manager);
        }

        info!(operation = "redis_pool_create", connection_id = %config.id, name = %config.name, host = %config.host, db, "creating connection manager");
    ...
    fn enforce_pool_limit(&self, conn_id: &ConnectionId, keep: &PoolKey, generation: u64) {
        ...
        if evicted > 0 {
            debug!(operation = "redis_pool_evict", connection_id = %conn_id, evicted, "database pools evicted by LRU");
        }
    }
    ...
    pub fn evict_all_dbs(&self, conn_id: &ConnectionId) {
        ...
        if evicted > 0 {
            info!(operation = "redis_pool_evict", connection_id = %conn_id, evicted, "connection pools evicted");
        }
    }
}
...
async fn build_connection_manager(config: &ConnectionConfig, db: u8) -> Result<ConnectionManager> {
        ...
        && let Some(ca) = config.ca_cert_path.as_deref().filter(|s| !s.is_empty())
    {
        ...
        .map_err(|e| {
        warn!(operation = "redis_pool_create", stage = "tls", error = %e, host = %config.host, "build TLS client failed");
            map_redis_error(e)
        })?
    } else {
        Client::open(info).map_err(|e| {
        warn!(operation = "redis_pool_create", error = %e, host = %config.host, "build client failed");
            map_redis_error(e)
        })?
    };
    ...
    .await
    .map_err(|e| {
        warn!(operation = "redis_pool_create", stage = "open", error = %e, host = %config.host, "open connection manager failed");
        map_redis_error(e)
    })?;
...
fn build_connection_info(
    ...
    let addr = if config.tls {
        ...
        if insecure && config.ssh_target.is_some() {
            warn!(operation = "redis_pool_tls_policy", host = %config.host, "TLS verification disabled over SSH tunnel");
        }
