...
use ramag_domain::entities::{DataSyncStage, DataSyncTaskId};
...
impl DataSyncOverlay {
    ...
    fn render_card(
        ...
        let theme = cx.theme();
        let (background, border, muted, foreground, danger) = (
            theme.background,
            theme.border,
        ...
        let counts = format!(
            ...
            progress.scanned, progress.inserted, progress.skipped, progress.failed
        );
        let details = format!(
            "传输 {} bytes    警告 {}    已用时 {:.2}s",
            progress.bytes,
        ...
        let result = snapshot.summary.as_ref().map(|summary| {
            format!(
                "扫描 {}，新增 {}，跳过 {}，失败 {}，耗时 {:.2}s",
                summary.scanned,
                summary.inserted,
            ...
            .child(
                v_flex()
                    ...
                    .when_some(result, |card, result| {
                        card.child(div().text_sm().child(result))
                    })
